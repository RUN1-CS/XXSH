...
// Function to parse an operand, which can be a literal or a variable, and store
// its value in a Variable struct
void parse_operand(char *operand, Variable *value, Variable *variables,
    ...
    if (operand[0] >= '0' && operand[0] <= '9') {
        ...
        strncpy(value->value.string_value, str_literal,
                sizeof(value->value.string_value));
...
// Checking for literals (numbers, booleans, and string literals) to
...
bool handle_condition(char line[MAX_COMMAND_LENGTH], Variable *variables,
    ...
    if (sscanf(line, "%7s %255s %7s %255s", type, left, op, right) < 1)
        return false;
    ...
    if (strcmp(type, "else") == 0) {
        return true;
               ...
               strcmp(type, "elif") == 0) { // but these mfs ain't
