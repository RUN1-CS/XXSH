...
void run_script(const char *filename, bool *running, Variable *variables,
                int *var_count) {
    ...
    // Counter for opened 'if' statements to detect unclosed blocks at the end
    // of the script
    int opened_ifs = 0;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        while (segment != NULL) {
            ...
            while (command != NULL) {
                ...
                // If we are currently skipping commands until we find an
                // 'endif', we just ignore everything until then
                if (skip_until_endif) {
                    command = strtok_r(NULL, ";", &cmd_save);
                    continue;
                }
                // Setting a variable is also a special command that doesn't
                // require forking, so we handle it here as well
                if (strncmp(trimmed, "set", 3) == 0) {
                    set_variable(trimmed, variables, var_count);
                    command = strtok_r(NULL, ";", &cmd_save);
                    continue;
                }
                // And ofc we have to handle increase and decrease commands for
                // numbers, otherwise it would be a pain to do it with 'set'
                // every time
                if (strncmp(trimmed, "increase", 8) == 0 ||
                    strncmp(trimmed, "inc", 3) == 0) {
                    char var_name[64];
                    if (sscanf(trimmed, "%*s %63s", var_name) == 1) {
                        for (int i = 0; i < *var_count; i++)
                            if (strcmp(variables[i].name, var_name) == 0 &&
                                variables[i].type == VAR_NUMBER) {
                                variables[i].value.number_value++;
                                break;
                            }
                    }
                    command = strtok_r(NULL, ";", &cmd_save);
                ...
                if (strncmp(trimmed, "decrease", 8) == 0 ||
                    strncmp(trimmed, "dec", 3) == 0) {
                    char var_name[64];
                    if (sscanf(trimmed, "%*s %63s", var_name) == 1) {
                        for (int i = 0; i < *var_count; i++)
                            if (strcmp(variables[i].name, var_name) == 0 &&
                ...
                if (strncmp(trimmed, "echo", 4) == 0) {
                    ...
                    while (*echo_content &&
                           isspace((unsigned char)*echo_content))
                        echo_content++;
                    if (*echo_content == '\0') {
                        // nothing to echo
                        command = strtok_r(NULL, ";", &cmd_save);
                    ...
                    if (echo_content[0] == '"' || echo_content[0] == '\'') {
                        ...
                        size_t len =
                            strnlen(echo_content + 1, sizeof(str_literal) - 1);
                        ...
                        if (sscanf(echo_content, "%63s", var_name) == 1) {
                            Variable var =
                                get_variable(variables, var_count, var_name);
                            switch (var.type) {
                            case VAR_STRING:
                                if (var.value.string_value[0])
                ...
                if (strncmp(trimmed, "export_path", 11) == 0) {
                    ...
                    if (sscanf(trimmed, "export_path %[^\n]", value) == 1) {
                        ...
                        printf(
                            "\033[1;31mUsage: export_path <new_path>\033[0m\n");
                    }
                    command = strtok_r(NULL, ";", &cmd_save);
                    continue;
                }
                ...
                if (strncmp(trimmed, "if ", 3) == 0 ||
                    (handle_elif && strncmp(trimmed, "elif", 4) == 0)) {
                    if (strncmp(trimmed, "if ", 3) == 0)
                        opened_ifs++;
                    bool condition_ok =
                        handle_condition(trimmed, variables, var_count);
                    if (!condition_ok) {
                ...
                if (strncmp(trimmed, "loop", 4) == 0) {
                    if (!handle_loop(trimmed, variables, var_count)) {
                        ...
                        strncpy(loops[loop_count].condition, trimmed,
                                sizeof(loops[loop_count].condition) - 1);
                        loops[loop_count]
                            .condition[sizeof(loops[loop_count].condition) -
                                       1] = '\0';
                        loop_count++;
                        loop_active = true;
                    }
                ...
                if (loop_active && strncmp(trimmed, "endloop", 7) == 0) {
                    if (handle_loop(loops[loop_count - 1].condition, variables,
                                    var_count)) {
    ...
    if (opened_ifs > 0) {
        printf("\033[1;31mError: %d unclosed 'if' statement(s) detected in "
               "script %s\033[0m\n",
               opened_ifs, filename);
    }
    fclose(file);
