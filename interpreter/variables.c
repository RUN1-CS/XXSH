...
#ifndef VAR_LIMIT
#define VAR_LIMIT                                                              \
    100 // Later will be dynamic, I am just too lazy to implement that right now
#endif
...
void set_variable(const char *line, Variable *variables, int *var_count) {
    ...
    for (int i = 0; i < *var_count; i++) {
        if (strcmp(variables[i].name, name) == 0) {
            ...
            case VAR_STRING:
                strncpy(variables[i].value.string_value, value,
                        sizeof(variables[i].value.string_value));
                break;
            case VAR_NUMBER:
    ...
    if (*var_count < VAR_LIMIT) {
        strncpy(variables[*var_count].name, name,
                sizeof(variables[*var_count].name));
        switch (type[0]) {
        case 's':
            variables[*var_count].type = VAR_STRING;
            strncpy(variables[*var_count].value.string_value, value,
                    sizeof(variables[*var_count].value.string_value));
            break;
        case 'n':
        ...
        case 'b':
            ...
            variables[*var_count].value.bool_value =
                (strcmp(value, "true") == 0);
            break;
        }
        (*var_count)++;
